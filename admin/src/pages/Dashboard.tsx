...
export default function Dashboard() {
  ...
  const stat = (valor: number, label: string, color: string) => (
    <div style={{ background: '#fff', border: '1px solid #e2e0da', padding: '20px 24px', borderTop: `3px solid ${color}` }}>
      <div style={{ fontFamily: "'IBM Plex Serif', Georgia, serif", fontSize: '36px', fontWeight: 700, color: '#111110' }}>{valor}</div>
  ...
  return (
    <div style={{ padding: '32px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px' }}>
        <div>
          <h1 style={{ fontFamily: "'IBM Plex Serif', Georgia, serif", fontSize: '28px', fontWeight: 700, color: '#111110', marginBottom: '4px' }}>Dashboard</h1>
          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', color: '#9c9a94' }}>
            {new Date().toLocaleDateString('es-AR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
          </div>
        </div>
        <Link to="/noticias/nueva" style={{ background: '#e8000d', color: '#fff', padding: '10px 20px', fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', fontWeight: 500, letterSpacing: '.1em', textTransform: 'uppercase', textDecoration: 'none' }}>
          + Nueva noticia
        </Link>
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e0da' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #e2e0da', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <h2 style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', fontWeight: 500, letterSpacing: '.15em', textTransform: 'uppercase', color: '#111110' }}>Últimas noticias cargadas</h2>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#f7f6f3' }}>
              {['Título', 'Sección', 'Estado', 'Fecha', 'Acciones'].map((h) => (
                <th key={h} style={{ padding: '10px 20px', fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '.1em', textTransform: 'uppercase', color: '#6b6b68', textAlign: 'left', borderBottom: '1px solid #e2e0da' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {ultimas.map((n) => (
              <tr key={n.id} style={{ borderBottom: '1px solid #f0ede8' }}>
                <td style={{ padding: '12px 20px', fontFamily: "'IBM Plex Sans', sans-serif", fontSize: '13px', color: '#111110', maxWidth: '300px' }}>
                ...
                <td style={{ padding: '12px 20px', fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: '#9c9a94' }}>
